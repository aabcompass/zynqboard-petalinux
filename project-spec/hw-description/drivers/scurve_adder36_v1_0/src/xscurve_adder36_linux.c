...
typedef struct {
    ...
    XScurve_adder36_uio_map maps[ MAX_UIO_MAPS ];
} XScurve_adder36_uio_info;

/***************** Variable Definitions **************************************/
static XScurve_adder36_uio_info uio_info;
...
static int uio_info_read_name(XScurve_adder36_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XScurve_adder36_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XScurve_adder36_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XScurve_adder36_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XScurve_adder36_Initialize(XScurve_adder36 *InstancePtr, const char* InstanceName) {
	XScurve_adder36_uio_info *InfoPtr = &uio_info;
